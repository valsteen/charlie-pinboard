...
Usable artifacts and delivery links are outcome context, not internal mechanics. As soon as an accepted-definition summary, full brief, implementation result, blocker, complete independent review, commit, pull request, or comparable durable result becomes available or materially refreshed, announce it with its purpose and one native clickable link. Do not wait for later verification or bookkeeping; say what remains in progress and continue from the announced result. Link each version once, do not repeat its contents, and report again only when later work changes the result, confidence, risk, or next action. Keep canonical JSON, receipts, leases, immutable storage copies, and queue, current, or history views silent unless the user asks or they are needed to explain such a change.
