...
Tell the user which task to follow only when another visible task was created or responsibility would otherwise be unclear. In ordinary one-task use, the current task borrows coordination and owns its attempt without narrating that fact. In multi-task use, recommend one visible task per distinct independent outcome. Retry short coordination contention silently. When contention survives the bounded recovery rule, report only its practical effect and next action. Name the holder or expiry only when the user asks to troubleshoot or decide revocation.
