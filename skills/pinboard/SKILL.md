...
4. Require the returned authority to be exactly `sqlite-v2`. If validation fails or another authority is reported, stop state-consuming work and use the recovery procedure in `references/state-and-recovery.md`; never infer a fallback from generated views or archived files.
...
1. Read `pinboard item definition --item-id <item> --json`, acquire the item's initial preparation claim under live coordination with that exact project revision, item subject revision, definition revision, and digest, then release coordination. Do this before loading implementation sources or compiling a brief. The item remains ready. Renew the exact preparation lease before it expires throughout source loading, publication, correction, and review. If work stops, release it. After expiry, release, or revocation, reacquire only through coordinated `preparation transfer`; re-read the definition and republish and rereview the brief whenever the transferred pin differs.
2. Compile one strict `pinboard-work-brief/v2` JSON candidate pinned to the preparation claim's exact definition revision and digest and containing the checkpoint, named source selectors, criteria, and exact checks. Run `pinboard brief publish --file <candidate> --json`. The accepted `.json` artifact is the sole semantic brief. Its generated Markdown attempt view is read-only convenience output and must never be edited or parsed as authority.
...
5. Before launching a cross-boundary checkpoint, read [references/brief-preservation.md](references/brief-preservation.md) completely. Plan the complete authority read with `pinboard brief-sources` before loading source bodies, then compile the reviewed-authority inventory, authoritative coverage, authorization basis for every Contract row and mandatory Verification entry, and lifecycle disposition from the exact named sources. Commission one fresh-context read-only reviewer to test every cheapest counterexample, including the declared architecture impact, the semantic role of every source-derived authorization basis, and whether each mandatory check's tool, threshold, platform, compatibility obligation, or hardening target is supported by accepted scope or selected source bytes. Publish digest-bound ready evidence. When coverage or authorization is missing or ambiguous, correct the brief before implementation; do not file a code defect for code that does not exist.
...
6. Select the exact `activate:<item>` action as Role.PREPARER with the preparation lease identity and generation. Activate with the reviewed brief reference; the transaction rechecks its complete identity, consumes the preparation claim, and creates the attempt. Only after the returned state is active, acquire the attempt lease and select the exact `dispatch:<attempt>` action. Record only `schema`, `checkout`, `branch`, `starting_revision`, and `permissions` in a `pinboard-dispatch/v1` environment JSON file. Permission values are `repository-read`, `repository-write`, `network`, `external-write`, and `live-application`.
7. Run `pinboard dispatch` with the action tokens, stable checkpoint ID, and environment file. Launch the worker with the rendered prompt unchanged. Use `--prompt` to verify a prompt received through another transport before launch. Never dispatch a ready item or a claimless start.
...
10. After completion, read `result.md` or `blocker.md`, inspect the stable candidate, and follow the current-responsibility review route above.
