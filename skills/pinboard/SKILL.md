...
3. If the default `.codex/pinboard` root is absent, report `WORKFLOW_UNAVAILABLE`. Initialize it with `pinboard init` only when the user explicitly requests coordinated-work setup. An explicit `--work-root` remains an exact user-selected exception. A first-run Codex setting line is optional advice about the user default; never edit configuration or infer the effective project setting from it. Do not ask for a permanent coordinator identity.
...
Tell the user which task to follow. In one-task use, that task borrows coordination and owns its attempt. In multi-task use, recommend one visible task per distinct independent outcome. If coordination or an attempt is busy, name the current holder and expiry, explain what can continue offline, and ask about revocation only when waiting is unsuitable.
