...
Coverage is invariant even when the efficient analyzer path is unavailable. For a repository without production Python, or for a language-specific category not covered by an existing analyzer, use the language-neutral structural sweep in the developer-navigation reference. Its mechanical fingerprint ledger must reconcile an independently counted construct universe as fingerprinted, explicitly excluded, or failed, with zero extraction failures; an extractor-defined eligibility count, header inventory, or representative read is incomplete. The fallback may cost more inspection, but it must produce the same category-by-category candidate or zero-result receipts. Never translate “unsupported by this helper” into “not required for this language.”
