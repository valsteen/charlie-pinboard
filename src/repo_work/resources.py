...
from repo_work.atomic import atomic_write_text
from repo_work.authority import Authority, AuthorityVersion, authority_transaction
from repo_work.leases import LeaseError, LeaseStatus, require_attempt, require_coordination
from repo_work.markdown import ITEM_PATTERN, parse_header, render_v2_header
...
def _v2_root(authority: Authority) -> Path:
    if authority.version != AuthorityVersion.V2:
        raise ResourceError(
            "MIGRATION_REQUIRED",
            "Resource operations require schema v2; run 'repo-work migrate --to v2' first.",
        )
