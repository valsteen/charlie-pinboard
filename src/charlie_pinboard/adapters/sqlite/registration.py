...
def _reject_legacy_state(work_root: Path) -> None:
    existing = tuple(selector for selector in _LEGACY_SELECTORS if (work_root / selector).exists())
    if existing:
        raise InitializationError(
            "MIGRATION_REQUIRED",
            f"Legacy work state requires the explicit importer before SQLite initialization: {', '.join(existing)}.",
        )
...
def initialize_work_state(
    ...
    *,
    ...
    try:
        ...
        _reject_legacy_state(roots.work_root)
