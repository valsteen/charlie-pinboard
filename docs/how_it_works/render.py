...
Charlie looks like a small workflow engine: work appears, moves, pauses, resumes, and eventually leaves the live queue. The consequential part is not moving a label from one column to another. Charlie preserves what the project decided, which execution is acting on it, who may change it now, and what evidence another task can trust later.
...
def main(arguments: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate the visitor-facing How Charlie works guide.")
